...
function loadcars(items_page = 2, total_prod = 0) {
    ...
    if(filtersSolucion != 0){ 
        var url = 'index.php?module=shop&op=list_filters_products';
        var post = {items_page: items_page, total_prod: total_prod, brand: brand, 
                    kilometros: kilometros, type: type, setting: setting, category: category, order: order};                                       
    }else if(filtersSolucion_search != 0){
        var url = 'index.php?module=shop&op=search_Menu';
        var post = {items_page: items_page, total_prod: total_prod, brand_search: brand_search, 
                    category_search: category_search, autocom_search: autocom_search};                                                                                                           
    }else{
        var url = 'index.php?module=shop&op=list_products';
        var post = {items_page: items_page, total_prod: total_prod};
    }
    selectFilters();
    ajaxPromise(url,
    'POST', 'JSON', post)
...
function load_details(){
    ...
    ajaxPromise('index.php?module=shop&op=details',
    'POST', 'JSON', {id: id})
    .then (function(data) {
        console.log(data);
        for (row in data) {
            $('<div></div>').attr('class',"carousel__elements").attr('id', data[1][0].id).appendTo(".carousel__shop").html( 
                "<img class='carousel__img' id='' src='"+ data[1][row].img_car +"' alt='' style = 'max-width: 100%; max-height: 600px;'>"
            )
        }
        ...
        $('<div></div>').attr('class',"").attr('id', data[0][0].id).appendTo(".Form_Aside_Car").html( 
        ...
        $('<div></div>').attr('class',"").attr('id', data[0][0].id).appendTo(".more_related").html( 
...
function load_pagination(){
    ...
    if(filtersSolucion != 0){ 
        var url = 'index.php?module=shop&op=pagination_filters';
        var post = {brand: brand, kilometros: kilometros, type: type, 
                    setting: setting, category: category, order: order};
    }else if(filtersSolucion_search != 0){
        var url = 'index.php?module=shop&op=count_search_Menu';
        var post = { brand_search: brand_search, category_search: category_search, autocom_search: autocom_search}; 
        filtersSolucion_search = 0;
        localStorage.setItem('filtersSolucion_search', JSON.stringify(filtersSolucion_search));                                                                                                                
    }else{
        var url = 'index.php?module=shop&op=pagination';
    }
    //selectFilters();
    ajaxPromise(url,
    'POST', 'JSON', post)
